...
#build titel review dict
def build_title_review_dict(reviews: List[Review]) -> Dict[str, Review]:
  dict = {}
  for review in reviews:
    dict.update({review.title: review})
  return dict


#review by title dict
@print_time_taken
def find_review_by_title_dict(
  ...
  for review in reviews_dict:
    if title in review:
      return reviews_dict[review]


#review by title list
@print_time_taken
...
def print_review(review: Review) -> None:
  print("\n".join(f"    {field}: {value}"
                  for field, value in asdict(review).items()))
